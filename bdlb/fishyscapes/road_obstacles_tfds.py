...
class RoadObstaclesTFDS(GeneratorBasedBuilder):
        ...
        return DatasetInfo(
            ...
            features=tfds.features.FeaturesDict(
                dict(
                    ...
                    # Labels source:
                    labels_source=
                    img_x1,  # 0 = background, 1 = road, 2 = anomaly, 255 = ignore
                    instances=img_x1,
    ...
    def get_frame(cls, fid: str, data_dir: Path):
        ...
        channels['mask'] = cls.convert_labels_for_onroad_benchmark(
            channels['labels_source'])
    ...
    def _split_generators(self, dl_manager: tfds.download.DownloadManager):
        ...
        if download_server is None:
            raise RuntimeError(
                'Please specify server URL as ROAD_OBSTACLE_URL env variable.')

        v = self.builder_config.version
        download_url = download_server + "/dataset_RoadObstacle_{v}.zip".format(
            v=v)
        ...
        make_split_entry = lambda name, key: SplitGenerator(
            name=name, gen_kwargs=dict(data_dir=str(data_dir), split=key))

        return [make_split_entry(tfds.Split.TEST, 'full')
                ] + [make_split_entry(k, k) for k in sorted(splits.keys())]

    def _generate_examples(self, data_dir,
                           split):  #-> Iterator[Tuple[Key, Example]]:
